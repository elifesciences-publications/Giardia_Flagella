...
param_data = pd.read_excel('E:\\Dawson_Lab\\Projects\\Flagella\\IFT_GFP\\IFT FRAP\\data\\190930_FRAP_fit_parameters.xlsx')

#setup aspects of the data to plot as a bar chart
x_pos = np.arange(len(param_data['region']))
diff_app = param_data['D_mean']
diff_app_error = param_data['D_sem']
...
with sns.axes_style('white'):
    ...
    ax.set_ylabel(u'Effective diffusion constant (${\mu}m^2$/sec)', fontsize=20)
    ax.set_xticks(x_pos[:])
    ax.set_xticklabels(param_data['region'][:], fontsize=20)
    plt.ylim([0,0.075])
    plt.tight_layout()
    plt.savefig('191001_PF-FP_PF-CA_FRAP_Dapp_plot.svg') 
