...
def diffusion_fit_fp (t, C, b):
    ...
    '''
    diffusion_fp = b + (C*1.5 - b) * (1 - (1.5**2 / (1.5**2 + (4 * pi * 0.049 * t)) ) **0.5)
    return diffusion_fp 
...
pfca_cell_number = np.array(['cell1','cell2','cell3', 'cell4', 'cell5', 'cell6',\
...
#setup a range of times to fit through; must start at t=zero
fp_fit_t = PFFP_df['time'][1:f]-PFFP_df['time'][1]
#=============================================================================
#fit the time averaged data for the FP with the FP equation

pf_fp_f,pf_fp_cov = curve_fit(diffusion_fit_fp, fp_fit_t,\
                              PFFP_df['average_int'][1:f], p0)    

#make a quick plot to assess the fit
with sns.axes_style('white'):
    plt.figure(figsize=(8,8))
    plt.scatter(fp_fit_t,PFFP_df['average_int'][1:f], c='r')
    plt.plot(fp_fit_t, diffusion_fit_fp(fp_fit_t, *pf_fp_f),\
                 'k-', label='fit')
#    plt.savefig('180920_FPCA_scatter.svg')    
   
#============================================================================ 
#setup a range of times to fit through; must start at t=zero
ca_fit_t = PFCA_df['time'][1:f]-PFCA_df['time'][1]

#fit the time averaged data for the CA with the CA equation
   
pf_ca_f,pf_ca_cov = curve_fit(diffusion_fit_ca, ca_fit_t,\
                              PFCA_df['average_int'][1:f], p0)    

#make a quick plot to assess the fit
with sns.axes_style('white'):
    plt.figure(figsize=(8,8))
    plt.scatter(ca_fit_t,PFCA_df['average_int'][1:f])   
    plt.plot(ca_fit_t, diffusion_fit_ca(ca_fit_t, *pf_ca_f), 'k-', label='fit') 
#    plt.savefig('180920_PFCA_scatter.svg') 
...
scipy.stats.linregress(fp_fit_t[:fp_linear],\
         (diffusion_fit_fp(fp_fit_t[:fp_linear], *pf_fp_f)-\
         diffusion_fit_fp(fp_fit_t[:fp_linear], *pf_fp_f).min())/\
          diffusion_fit_fp(fp_fit_t[:fp_linear], *pf_fp_f).max())
...
scipy.stats.linregress(ca_fit_t[:ca_linear],\
         (diffusion_fit_ca(ca_fit_t[:ca_linear], *pf_ca_f)-\
         diffusion_fit_ca(ca_fit_t[:ca_linear], *pf_ca_f).min())/\
          diffusion_fit_ca(ca_fit_t[:ca_linear], *pf_ca_f).max())
...
with sns.axes_style('white'):
    plt.figure()
    plt.plot(ca_fit_t,\
         (diffusion_fit_ca(ca_fit_t, *pf_ca_f)-\
         diffusion_fit_ca(ca_fit_t, *pf_ca_f).min())/\
          diffusion_fit_ca(ca_fit_t, *pf_ca_f).max(),\
                          color='k',label='CA')
         
    plt.plot(ca_fit_t,(intercept_ca + slope_ca*ca_fit_t),\
             'k--', label='ca_linear_regression')
    
    plt.scatter(ca_fit_t,(PFCA_df['average_int'][1:f] -\
         diffusion_fit_ca(ca_fit_t, *pf_ca_f).min())/\
          diffusion_fit_ca(ca_fit_t, *pf_ca_f).max(),color='m',\
                          edgecolor='k',\
                          label='time averaged intensity')
    ...
    plt.xlim([0,300])
    plt.ylim([0,1])
    plt.savefig('191001_PFCA_linearfit_norm_scatter.svg')    


with sns.axes_style('white'):
    plt.figure()    
    plt.plot(fp_fit_t,\
         (diffusion_fit_fp(fp_fit_t, *pf_fp_f)-\
         diffusion_fit_fp(fp_fit_t, *pf_fp_f).min())/\
          diffusion_fit_fp(fp_fit_t, *pf_fp_f).max(),\
                          color='k', label='FP')
         
    plt.plot(fp_fit_t,(intercept_fp + slope_fp*fp_fit_t),\
             'k--', label='fp_linear_regression')
    
    plt.scatter(fp_fit_t,(PFFP_df['average_int'][1:f] -\
         diffusion_fit_fp(fp_fit_t, *pf_fp_f).min())/\
          diffusion_fit_fp(fp_fit_t, *pf_fp_f).max(), \
                           color='g', edgecolor='k',\
                           label='time averaged intensity')
    ...
    plt.xlim([0,300])
    plt.ylim([0,1])
    plt.savefig('191001_PFFP_linearfit_norm_scatter.svg')   
